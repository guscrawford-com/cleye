...
describe('CliApp',()=>{
    ...
    describe('runs specified "run" properties assigned functions',()=>{
        let defCommand, mockRunner = {
            get run() {
                return ()=>console.info('ran')
            }
        }, mockRunnerSpy:any;
        beforeEach(()=>{
            defCommand = {...commandDef,name:'default',run:mockRunner.run,options:{opt1:{name:'opt1',spinalCaseName:'opt1'}}};
            mockRunnerSpy = spyOnProperty(mockRunner, 'run')
            cliApp = new CliApp({commands:{
                default:defCommand
            }},mockProcessArgV.concat(['--aaa-bbb-ccc', 'abacus','--xxYyZz=expected-value', '--opt1']));
        });
        it('defaultCommand.run ran',()=>{
